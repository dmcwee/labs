...
@minLength(3)
param domainNetbiosName string = 'mcweeinc'
...
module dcModule '../modules/virtualMachine.bicep' = {
  params: {
    name: adServerName
    sku:'2016-Datacenter'
    offer: 'WindowsServer'
...
module servers '../modules/virtualMachine.bicep' = [for (server, i) in labServers: {
  params: {
    name: server.name
    osType: server.type
