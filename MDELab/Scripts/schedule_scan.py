...
def backup_cron_jobs(debug):
    try:
        ...
        backup_location = f"~/cron_backup_{formatted_date}"

        cron_backup = f"(crontab -l > ~/cron_backup_{formatted_date})"
        if debug == True:
...
def create_cron_job(minute="*", 
                    ...
                    month="*", 
                    day_of_week="*", 
                    command="/bin/mdatp scan quick > ~/mdatp_cron_job.log", 
                    debug=False):
    ...
    cron_export = f"crontab -l > /tmp/cron_schedule_{formatted_date}"
    cron_append_task = f"echo '{cron_expression}' >> /tmp/cron_schedule_{formatted_date}"

    # push cron task temp file back to crontab for loading
    cron_cmd = f"crontab /tmp/cron_schedule_{formatted_date}"
